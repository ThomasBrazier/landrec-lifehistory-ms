...
%DIF DEL Submission_JEB/main_anonymous.tex   Mon Oct 28 15:22:08 2024
%DIF ADD main_anonymous.tex                  Mon Oct 28 15:15:34 2024
...
We used the 'phytools' R package to manipulate phylogenetic tree data and to statistically test for a phylogenetic signal (\cite{revellPhytoolsUpdatedEcosystem2024}). The phylogenetic time-calibrated supertree used for the comparative phylogenetic dataset was retrieved from Smith and Brown (\cite{smithConstructingBroadlyInclusive2018}). Tip labels were curated to remove subspecies and cultivar annotations and one tip per species was retained. Species \DIFaddbegin \DIFadd{in our dataset but }\DIFaddend missing in the \DIFaddbegin \DIFadd{Smith and Brown reference }\DIFaddend tree were assigned randomly to a sister species of the same genus. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{As downstream comparative methods assume the tree is ultrametric (i.e. all branchs end at the same time), the }\DIFaddend tree was forced to be ultrametric. We computed two phylogenetic signal metrics, Blomberg's K and Pagel's lambda. \DIFaddbegin \DIFadd{Blomberg's K measures how similar trait values are among closely related species, compared to a Brownian motion model where changes accumulate at a constant rate over time. On the other hand, Pagel’s lambda measures how much the phylogenetic tree structure influences the trait. }\DIFaddend We also fitted three evolution models (Brownian Motion, Ornstein-Ulhenbeck and Early Burst) to the distribution of recombination rates, chromosome genetic map length and the residuals of the regression presented above.
...
After controlling for the chromosome size effect, we also investigated how recombination rates evolved amongst the plant phylogeny. In order to test the significance and strength of the phylogenetic signal, we estimated two phylogenetic signal metrics (Blomberg’s K and Pagel’s lambda) and fitted three competing phylogenetic models (Brownian Motion, Ornstein-Uhlenbeck and Early-burst) to the continuous distribution of chromosome average genetic length and the residuals of the regression of recombination rates as a function of average chromosome size (Table \ref{table:table1}). Blomberg's K \DIFdelbegin \DIFdel{were not }\DIFdelend \DIFaddbegin \DIFadd{was }\DIFaddend significantly different from zero \DIFdelbegin \DIFdel{while }\DIFdelend \DIFaddbegin \DIFadd{for the residuals by not the genetic map length. In both cases }\DIFaddend Pagel's Lambda was around 0.5\DIFdelbegin \DIFdel{, both indicating a weak phylogenetic signal}\DIFdelend \DIFaddbegin \DIFadd{. Taken together this is indicating a moderate phylogenetic signal, as the variation in recombination rates can be partially explained by the phylogeny}\DIFaddend . Based on the maximum Log-Likelihood criterion, the Ornstein-Uhlenbeck phylogenetic model was always preferred\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{. Contrary to the random walk in the Brownian motion model, traits are assumed to evolve towards an optimal value under the Ornstein-Uhlenbeck model, thus }\DIFaddend suggesting that the recombination rate evolved under a constrained range.
...
However, our dataset remains limited to detect complex effects. The interaction between chromosome size and the mating system is only a trend. We did not detect significant effects for other life-history traits, despite strong expectations for some of them (e.g. cultivation status). There is multiple evidence of an effect of domestication on recombination rates in plants\DIFdelbegin \DIFdel{(\mbox{%DIFAUXCMD
...
\DIFadd{We thank Laetitia Carrive for discussions about plant phylogeny and the two anonymous reviewers for their insightful comments that improved the manuscript.
}
...
\DIFadd{Agence Nationale de la Recherche, Grant ANR-19-CE12472 0019 / HotRec. 
}


\section*{\DIFadd{Author contributions}}

\textbf{\DIFadd{Thomas Brazier:}} \DIFadd{Conceptualization, Data curation, Formal analysis, Visualization, Writing - original draft, Writing - review \& editing. }\textbf{\DIFadd{Roman Stetsenko:}} \DIFadd{Writing - original draft, Writing - review \& editing. }\textbf{\DIFadd{Denis Roze:}} \DIFadd{Writing - original draft, Writing - review \& editing. }\textbf{\DIFadd{Sylvain Glémin:}} \DIFadd{Conceptualization, Data curation, Formal analysis, Funding acquisition, Visualization, Writing - original draft, Writing - review \& editing.
}
